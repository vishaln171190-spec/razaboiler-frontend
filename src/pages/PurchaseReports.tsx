...
const PurchaseReports: React.FC = () => {
  ...
  const canView = isAdmin || hasPermission(authUser, "report_purchase");
  ...
  React.useEffect(() => {
    Promise.all([fetchCompanies(), fetchItems()]).finally(() => setLoading(false));
  }, []);
  ...
  const getReport = async () => {
    ...
    const url = `${API_BASE_URL}/reports/purchase?companyid=${company}&start_date=${start_date}&end_date=${end_date}&itemid=${itemType}`;
    try {
      ...
      const rows = (data.data || data || []).map((row: any) => ({
        ...
        weight: row.parchaseweight || row.weight || 0,
        rate: row.rateofpurchase || row.rate || 0,
        total: row.total_amount || row.total || (row.purchase_weight * row.purchase_rate) || 0,
      }));
      setReportData(rows);
    } catch (err) {
  ...
  const exportExcel = () => {
    ...
    // Convert to CSV
    const csvRows = [
      ['Company', 'Item', 'Purchase Weight', 'Purchase Rate', 'Total Amount'],
      ...reportData.map((row: any) => [row.companyname, row.item, row.weight, row.rate, row.total]),
    ];
  ...
  if (!canView) {
    return <div className="p-8 text-center text-slate-500">You do not have permission to view Purchase Reports.</div>;
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-medium text-slate-500">Loading Purchase Report...</p>
        </div>
      </div>
    );
  }
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/80 backdrop-blur-md px-4 py-3 shadow-sm md:px-6">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold tracking-tight text-slate-800">Purchase Reports</h1>
          ...
          <div className="flex flex-1 items-center justify-end gap-3 md:gap-6"></div>
        </div>
      </header>
      <main className="mx-auto max-w-7xl p-4 md:p-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden">
          ...
          <div className="px-6 py-4 flex flex-wrap gap-4">
            <select value={company} onChange={e => setCompany(e.target.value)} className="w-64 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none">
            ...
            {dateFilter === 'customdate' && (
              <div className="flex gap-2 mt-2">
                <input type='date' value={customStart} onChange={e => setCustomStart(e.target.value)} className="h-10 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all cursor-pointer" />
                <input type='date' value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="h-10 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all cursor-pointer" />
              </div>
            )}
          </div>
          <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-2">
            <button onClick={getReport} disabled={loading} className="px-8 py-3 rounded-xl font-bold text-white shadow-lg transition-all flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400">
              {loading ? 'Loading...' : 'Get Report'}
            </button>
            <button onClick={exportExcel} className="px-8 py-3 rounded-xl font-bold text-white shadow-lg transition-all flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-400">
              Export
            </button>
          </div>
        </div>
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50 mt-6">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center gap-2">
            <h3 className="text-sm font-bold uppercase tracking-widest text-slate-500">Purchase Report</h3>
          </div>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead className="bg-slate-50/80 uppercase text-[10px] font-black tracking-widest text-slate-400 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center">Purchase Weight</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {reportData.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-400">No purchases found.</td>
                  </tr>
                ) : (
                  reportData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/30 transition-colors">
                      <td className="px-6 py-4 font-semibold text-slate-700 text-center">{row.companyname}</td>
                      <td className="px-6 py-4 font-semibold text-slate-700 text-center">{row.purchasedate}</td>
